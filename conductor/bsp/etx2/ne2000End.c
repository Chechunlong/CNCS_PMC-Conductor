...
#include "intLib.h"
#include "taskLib.h"
#include "ne2000End.h"		/* Common defines. */
	...
	)
{
	...
	while (pDrvCtrl->flags & END_RECV_HANDLING_FLAG)
	{
		...
		/* */
		if(pDrvCtrl->recvHook && (pDrvCtrl->recvHook(pDrvCtrl->unit, pBuf, len) == OK)){
	...
	)
{
	...
	*/
	...
	if (pDrvCtrl->flags & END_TX_IN_PROGRESS)
	{
		...
		/* Wait up to 10 mili-second */
		cnt = sysClkRateGet () / 100;
	...
	)
{
	...
	memcpy(pDrvCtrl->enetAddr, ne2000EnetAddr[pDrvCtrl->unit], 6);
	...
	)
{
	...
	*/
	...
	if (pDrvCtrl->flags & END_TX_IN_PROGRESS){
		...
		cnt = timeout*(1000/sysClkRateGet());
		while ((pDrvCtrl->flags & END_TX_IN_PROGRESS) && (cnt > 0)){
			taskDelay (1);
			cnt-=(1000/sysClkRateGet());
		}
	}
	...
	memcpy(pDrvCtrl->packetBuf, data, dataLen);
	len = max (dataLen, ETHERSMALL);
...
void	ne2kInt (int c)
{
	NE2000END_DEVICE* pDrvCtrl = (NE2000END_DEVICE *) ne2Resources[c].devCtrl;
	ne2000Int(pDrvCtrl);
}

#if 0
STATUS ne2000PollSendDirect
	(
	int c,
	void * data, /* data to send */
	int dataLen
	...
	)
{
	...
	*dataLen = 0;
	...
	len = ne2000PacketGet (pDrvCtrl, data);
	if (len <= 0){
		return ((STATUS) (EAGAIN));
	}
...
void ne2000PollFlushDirect
	(
	int c
	)
{
	static char data[NE2000_BUFSIZ];
	int len=NE2000_BUFSIZ;
	while(ne2000PollRecvDirect(c, data, &len)==OK){
		;
	}
}
#endif
