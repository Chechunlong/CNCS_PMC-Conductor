...
#ifdef INCLUDE_ATA
	/* 
	hack for VMWARE, the following file is modified, so we need to 
	include it here for the modifications to take affect otherwise the 
	precompiled version will be used.
	
	  by J.Hunter(jhunter@tsinghua.org.cn), 2004/1/2
	*/
#include "hdisk/ataDrv.c"
#endif
...
#if 0
#define LOGGED_CALL( x ) sysPrintf(#x "\n"), x, sysPrintf("call complete.\n")
#else
#define LOGGED_CALL( x ) x
#endif
...
void sysHwInit (void)
{
	...
	sysDebugTrace(sysProcessor);
	/* sysProcessor = X86CPU_NS486; */
	...
	/* by soycola, 2004/1, MSR not applicable for ETX board */
	LOGGED_CALL(pentiumMsrInit ());
#endif
...
#	else
		LOGGED_CALL((void) pentiumMtrrSet (&sysMtrr)); /* set your own MTRR */
...
#	endif /* INCLUDE_SHOW_ROUTINES */
	...
	/* soycola, ETX dies before this line, 2004/1/6 */
...
#if 0
	sysPrintf("Memory auto probe, pPage=%08x,pageSize=%08x,delta=%08x\n", pPage,pageSize, delta);
	sysDelayMs(2000);
#endif
		...
		{
#if 0
			sysPrintf("Probing %08x...", pPage);
			sysDelayMs(500);
#endif
			
			LOGGED_CALL(WRITE_MEMORY_TEST_PATTERN ((int *) pPage, &temp[0]));
