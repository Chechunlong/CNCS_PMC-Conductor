...
//** modified by cj @ 2010-4-20
typedef struct tag_name_value{
	TAG_NAME tn;
	TAG_VALUE tv;
	int type;
	int pulse;
...
static void waitLButtonUP()
{
	SHORT s=1;
	while(s!=0){			
		s=GetAsyncKeyState(VK_LBUTTON);		
		Sleep(100);
	}
}

void verifyWrite(PCTAG_NAME names, PCTAG_VALUE values)
{
	int done=0;
	for(int i=0; i<10 && done==0; i++){
		proxy_write_tags(1, names, values);
		for(int j=0; j<3 && done==0; j++){ // 最多重试3次
			Sleep(100);
			if(lock_rtdb(__false, 100)){
				PRTK_TAG p;
				p = query_tag_f(names);
				if(p){
					if(p->d.Value.Value.iValue==values->iValue){
						done=1;
					}
				}else{
					done=1;
...
static void threadWrite(void *p)
{
	TNV *pTNV=(TNV *)p;
	...
	if(pTNV->type==0){
		// 置位
		pTNV->tv.iValue=1;
		proxy_write_tags(1, &pTNV->tn, &pTNV->tv);	
	}else if(pTNV->type==1){
		// 复位
		pTNV->tv.iValue=0;
		proxy_write_tags(1, &pTNV->tn, &pTNV->tv);	
...
void CFView::delayWrite(PCTAG_NAME names, PCTAG_VALUE values, int type, int pulse)
{
	TNV *pTNV=new TNV;
	pTNV->tn=*names;
	pTNV->tv=*values;
	pTNV->type=type;
	pTNV->pulse=pulse;
	pTNV->p_bPulse=&m_bPulse;
	_beginthread(threadWrite, 1024, pTNV);
}
//**
	...
	if(pSelElement!=NULL){
		if(pSelElement->m_DnyAttr.m_en){
			if(pSelElement->m_DnyAttr.m_push==BUTTON_WND){
				...
				tv=pSelElement->m_DnyAttr.m_ButtonD.m_TagValue.Value;
// modified by cj @ 2004-4-20
				delayWrite(&tn, &tv, pSelElement->m_DnyAttr.m_ButtonD.m_Action, pSelElement->m_DnyAttr.m_ButtonD.m_pulseWidth);
				/*
				pmc_value_t tg;
				tg.Flags |= TF_Valid;
