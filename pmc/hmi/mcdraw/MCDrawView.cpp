...
void CMCDrawView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(m_lDrawMode==DE_LIBCOMP){
		IXMLDOMDocumentPtr pCellFile;
		HRESULT hr = pCellFile.CreateInstance(CLSID_DOMDocument);
		if (FAILED(hr)){
			 _com_error er(hr);
			...
			if(pCellFile->load(sCellFile.AllocSysString())==FALSE){
				...
				IXMLDOMElementPtr pRoot=pCellFile->documentElement;
	...
	if(m_lDrawMode==DE_PASTE){
		IXMLDOMDocumentPtr pPaste;	  
		HRESULT hr = pPaste.CreateInstance(CLSID_DOMDocument);
		if (FAILED(hr)){
			_com_error er(hr);
			AfxMessageBox(er.ErrorMessage());
		}else{
			pPaste->loadXML((LPCTSTR)m_paste);
			IXMLDOMElementPtr pRoot=pPaste->documentElement;
			//装载数据
			_bstr_t strGO=L"go";
			_variant_t strArc=L"arc";
			_variant_t strCell=L"cell";
...
void CMCDrawView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if(m_lEditMode==ED_EDIT){
		if(nChar==13){
			...
			((CText *)m_pCurElement)->m_text+=nChar;
