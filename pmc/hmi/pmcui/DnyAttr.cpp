...
void CDnyAttr::Save(IXMLDOMDocumentPtr pDoc, IXMLDOMElementPtr pDynamic)
{
    CString strTemp;
    _variant_t val;

	IXMLDOMElementPtr pSubAttr;
	IXMLDOMElementPtr pItem;

    //rank
    pSubAttr=pDoc->createElement(L"rank");
    ...
    if(m_ButtonD.m_Used){
        ...
        pSubAttr->setAttribute(L"m",val);
		val.vt = VT_I4;
		val.lVal= m_ButtonD.m_pulseWidth;
		pSubAttr->setAttribute(L"w",val);
    }
	...
	//button close popup
	if(m_ClsPop){
        pSubAttr=pDoc->createElement(L"button_cls_pop");
        pDynamic->appendChild(pSubAttr);
	}
    ...
    if(m_Visible.m_Used){
        ...
        //tagname
        val.vt=VT_BSTR;
        val.bstrVal=m_Visible.m_TagName.AllocSysString();
        pSubAttr->setAttribute(L"tag",val);
        //visible
        val.vt=VT_BOOL;
...
void CDnyAttr::Load(IXMLDOMElementPtr pDynamic)
{
	HRESULT hr;
	IXMLDOMElementPtr pSubAttr;
