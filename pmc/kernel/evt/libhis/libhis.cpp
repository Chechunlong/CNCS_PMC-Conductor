...
static inline name2id(char name[_MAX_FNAME], unsigned int *id)
{
...
HIS_API time2str(PMCTIME tm, char str[])
{
	PMCSYSTIME sysTm;
	tm2systm(&tm, &sysTm);
...
HIS_API str2time(char str[], PMCTIME *tm)
{
	PMCSYSTIME sysTm;
	char temp[5];
...
HIS_API tm2systm(const PMCTIME *tm, PMCSYSTIME *sysTm)
{
	FILETIME ftm;
	SYSTEMTIME stm;
...
HIS_API systm2tm(const PMCSYSTIME *sysTm, PMCTIME *tm)
{
	FILETIME ftm;
	SYSTEMTIME stm;
...
HIS_API markNewTimeId(char pool[_MAX_FNAME])
{
	char str[15];
	PMCTIME tm = getCurrentTime();
	time2str(tm, str);
