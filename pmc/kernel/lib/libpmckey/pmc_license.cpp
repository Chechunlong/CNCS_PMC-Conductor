...
void createNullLicenseFile(const char * file)
{
	FILE * fp;
	char localKey[16];
	int i;
...
*/
BOOL insertLicense(const char * file, struct pmc_license_t * lic)
{
	FILE * fp;
	int i;
...
*/
BOOL removeLicense(const char * file, int id, int count)
{
	if(!isLicenseFileValid(file)){
		return FALSE;
	}
...
/*
*/
int enumLicense(const char * file, ENUM_LIC_PROC p, int con)
{
	int i,id;
	__u8 key[16];
	int count=0;
