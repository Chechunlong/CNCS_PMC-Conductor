...
#undef __t
	...
	case dt_uint64:
		...
		element=args.argv[idx++];
		tte.s.fltAlarmDeadband=atof(element.data());
		if(tte.s.fltAlarmDeadband<0.001){
			// return -8;
		}

		// extrace fltHiHi,fltHi,fltLo,fltLoLo,
		tte.s.fltHiHi=atof(args.argv[idx++]);
		tte.s.fltHi=atof(args.argv[idx++]);
		tte.s.fltLo=atof(args.argv[idx++]);
		tte.s.fltLoLo=atof(args.argv[idx++]);
		tte.s.fltRate=atof(args.argv[idx++]);
		break;
	case dt_int8:
	case dt_int16:
	case dt_int32:
	...
	case dt_uint32:
		...
		tte.s.fltRate = atoi(args.argv[idx++]);
		break;
	case dt_bool:
	case dt_date:
		idx+=6;
	...
	switch(get_value_type(tte.s.Flags)){
		...
		case dt_uint64:
			...
			tte.s.fltMinValue = atof(args.argv[idx++]);
			tte.s.fltMaxValue = atof(args.argv[idx++]);
			break;
	}
	...
	if(args.argc >= 27){
		tte.s.ExcDev = atof(args.argv[idx++]);
		...
		tte.s.CompDev = atof(args.argv[idx++]);
	}
	...
	)
{
	...
	*/
	...
	if(type=='f'){
		// fltAlarmDeadBand
		s->fltAlarmDeadband=rng_rand(1,5)*0.01;
