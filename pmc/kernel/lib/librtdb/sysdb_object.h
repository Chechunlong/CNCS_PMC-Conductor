...
public:
	...
	*/
	...
	static inline proxy_class * create(
		...
		const TYPENAME table_class::referent_type ref
		)
	{
...
#define TAG_HANDLE_TAG ((__uint)('G' << 24) + ('A' << 16) + ('T' << 8) + 'H')
#define GRP_HANDLE_TAG ((__uint)('P' << 24) + ('R' << 16) + ('G' << 8) + 'H')
#define NOD_HANDLE_TAG ((__uint)('D' << 24) + ('O' << 16) + ('N' << 8) + 'H')
