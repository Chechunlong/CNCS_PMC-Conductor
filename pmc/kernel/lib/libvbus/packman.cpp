...
/*
	2005-10-28 en-large these two values, jht
*/
...
{
	char	packet_buffer[VBUS_MAX_PACKET_SIZE];
	PRTK_PACKET packet;
	__u16 *rsids;
	__uint items;
	int j, i;
	...
	*((PRTK_GUID)packet->data) = h->header->guid;
	...
	dbg_p = dbg_msg;
	dbg_p += sprintf(
		dbg_p, "_P: Requesting %d of %d, grp %d:", 
		items, 
		h->header->total_frag,
		h->header->grp_id
		);
	packet->guid.to_string(guidName);
	dbg_p += sprintf(dbg_p, "[%s] -- ", guidName);
#endif
