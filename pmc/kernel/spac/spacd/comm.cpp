...
#define lock_port(t, tout) __true
#define unlock_port(t)
#define init_spac()
#define uninit_spac()
#define read_port(pno, buf, size, tout)	sio_read(pno, buf, size)
#define write_port(pno, buf, size, tout)	sio_write(pno, buf, size)

CSerialPort::CSerialPort(int pno)
{
